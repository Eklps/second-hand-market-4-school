...
- 当前登出策略（前端删除Token + 服务端清理上下文）与可扩展黑名单方案
...
| UV 统计 | **HyperLogLog** | `post:uv:{postId}` | `PFADD` 去重计数，误差约 0.81% |
...
| 特性 | 实现 |
|---|---|
| 全局异常兜底 | `@RestControllerAdvice` + `@ExceptionHandler(RuntimeException.class)` |
| 错误日志追踪 | 统一 `log.error(e.toString(), e)` |
| 统一返回结构 | 统一封装为 `Result.fail(...)` |

**涉及文件**：[WebExceptionAdvice.java](file:///e:/DianPing/secondHandMarket/2handMarket/src/main/java/com/hmdp/config/WebExceptionAdvice.java)、[Result.java](file:///e:/DianPing/secondHandMarket/2handMarket/src/main/java/com/hmdp/dto/Result.java)

### 9.2 框架中间件配置

| 特性 | 实现 |
|---|---|
| MyBatis-Plus 分页插件 | `MybatisPlusInterceptor + PaginationInnerInterceptor(DbType.MYSQL)` |
| 时间序列化统一 | `Jackson2ObjectMapperBuilderCustomizer` 统一 `LocalDateTime` 格式 |
| WebSocket 端点注册 | `ServerEndpointExporter` 自动注册 `@ServerEndpoint` |

**涉及文件**：[MybatisConfig.java](file:///e:/DianPing/secondHandMarket/2handMarket/src/main/java/com/hmdp/config/MybatisConfig.java)、[MvcConfig.java](file:///e:/DianPing/secondHandMarket/2handMarket/src/main/java/com/hmdp/config/MvcConfig.java)、[WebSocketConfig.java](file:///e:/DianPing/secondHandMarket/2handMarket/src/main/java/com/hmdp/config/WebSocketConfig.java)

### 9.3 生命周期与后台任务治理

| 特性 | 实现 |
|---|---|
| 启动预热 | `CommandLineRunner` 启动时批量预加载商品 GEO 数据 |
| 后台消费线程启动 | `@PostConstruct` 初始化 Redis Stream 消费任务 |
| 优雅关闭 | `@PreDestroy` 关闭秒杀异步消费线程池 |

**涉及文件**：[ProductGeoDataLoader.java](file:///e:/DianPing/secondHandMarket/2handMarket/src/main/java/com/hmdp/config/ProductGeoDataLoader.java)、[VoucherOrderServiceImpl.java](file:///e:/DianPing/secondHandMarket/2handMarket/src/main/java/com/hmdp/service/impl/VoucherOrderServiceImpl.java)
...
|---|---|
| 防重名 | UUID 文件名 |
| 目录打散 | 帖子图片基于哈希分桶 `/posts/{d1}/{d2}/...`，降低单目录文件数 |
| 分类存储 | 头像单独存放 `/icons/` |
| 删除保护 | 删除前判断 `isDirectory`，防止目录误删 |

**面试亮点**：
- 为什么上传目录要“分桶打散”（文件系统性能、运维便利）
- 统一上传入口如何兼顾帖子图与头像图两类场景

**涉及文件**：[UploadController.java](file:///e:/DianPing/secondHandMarket/2handMarket/src/main/java/com/hmdp/controller/UploadController.java)、[SystemConstants.java](file:///e:/DianPing/secondHandMarket/2handMarket/src/main/java/com/hmdp/utils/SystemConstants.java)
...
| 场景 | 一致性策略 |
|---|---|
| 关注/取关 | `@Transactional` 保证 DB 关注关系与统计字段同步 |
| 商品更新 | 先更新 DB，再删除缓存，避免脏读 |
| 秒杀异步下单 | Stream 消费成功后 ACK，异常进入 PendingList 重试 |

**面试亮点**：
- 事务一致性与 Redis 最终一致性的边界划分
- 为什么采用“删缓存”而不是“更新缓存”

**涉及文件**：[FollowServiceImpl.java](file:///e:/DianPing/secondHandMarket/2handMarket/src/main/java/com/hmdp/service/impl/FollowServiceImpl.java)、[ProductServiceImpl.java](file:///e:/DianPing/secondHandMarket/2handMarket/src/main/java/com/hmdp/service/impl/ProductServiceImpl.java)、[VoucherOrderServiceImpl.java](file:///e:/DianPing/secondHandMarket/2handMarket/src/main/java/com/hmdp/service/impl/VoucherOrderServiceImpl.java)
...
| 工程化 | 全局异常处理 + MyBatis 分页插件 + Jackson 时间序列化 |
